...
(function ($) {
    ...
    function r_fun_ajax(fun, settings, handler) {
        ...
        var jqxhr = $.ajax(settings).done(function () {
            var loc = jqxhr.getResponseHeader('Location') || warn("Location response header missing.");
            var key = jqxhr.getResponseHeader('X-ocpu-session') || warn("X-ocpu-session response header missing.");
